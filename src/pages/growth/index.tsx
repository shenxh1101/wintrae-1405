...
const GrowthPage: React.FC = () => {
  const { eyeCareMode, growthRecords, getGrowthStats } = useApp();
  ...
  const sortedRecords = useMemo(() => {
    return [...growthRecords].sort((a, b) => b.date.localeCompare(a.date));
  }, [growthRecords]);
  ...
  const handleShareReport = () => {
    ...
    const hasData = growthRecords.length > 0;
    if (!hasData) {
      Taro.showToast({
        title: '先读一本书再来分享吧~',
        icon: 'none'
    ...
    Taro.showModal({
      title: '生成阅读小报',
      content: `本月已阅读 ${stats.totalDays} 天，共 ${stats.totalBooks} 本绘本，合计 ${stats.totalMinutes} 分钟！是否生成分享？`,
      confirmText: '生成',
      success: (res) => {
        if (res.confirm) {
